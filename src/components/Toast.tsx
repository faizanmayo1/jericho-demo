...
const KIND_STYLE: Record<ToastKind, { border: string; icon: ReactNode }> = {
  ok: { border: 'border-l-plum', icon: <Check className="h-3.5 w-3.5 text-plum-deep" /> },
  ai: { border: 'border-l-almanac', icon: <Sparkles className="h-3.5 w-3.5 text-almanac-deep" /> },
  warn: { border: 'border-l-change', icon: <TriangleAlert className="h-3.5 w-3.5 text-change-deep" /> },
}
      ...
      <div className="pointer-events-none fixed bottom-5 right-5 z-[60] flex w-full max-w-sm flex-col gap-2">
        {items.map((t) => (
          <div
            key={t.id}
            className={cn(
              'pointer-events-auto animate-rise rounded-card border border-l-[3px] border-line bg-surface p-3 shadow-pop',
